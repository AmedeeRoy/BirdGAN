  ...
  SVtracks <- read.csv("./../data/trips_SV_2008_2015.csv", stringsAsFactors = FALSE)
  SVtracks <- SVtracks[seq(1, nrow(SVtracks), by = 4),c("trip", "lon", "lat", "prediction")]
  
  ## select few tracks
  colnames(SVtracks) <- c("ID", "lon", "lat", "dive")
  select = unique(SVtracks$ID)[1:78]
  ...
  hist(data$dive, freq = FALSE)
  x <- seq(0, 1, by = 0.01)
  lines(x, dbeta(x,1,5), col = 'green')
  lines(x, dbeta(x,10,3), col = 'blue')
  lines(x, dbeta(x,1,20), col = 'red')
  
  # search    forage   inbound
  # mean 0.8025364 0.3451937 0.9924471
  # sd   0.1157530 0.2968860 0.1144059
  
  ### Model 1 : no covariates
  stateNames <- c("travel","forage", "dive", "inbound")
  
  # initial parameters
  stepPar0 <- c(0.8, 0.6, 0.3, 1 ,
                0.1,0.2,0.2, 0.05)
  anglePar0 <- c(30,6,0.5, 50)
  divePar0 <- c(1, 1, 10, 1,
                5, 5, 3, 20)
  
  # constrain transition probabilities
  fixbeta <- matrix(c(NA,-100, NA, NA, NA, NA, NA, NA, NA, -100, -100, -100),
                    nrow=1)
  
  m1 <- fitHMM(data=data, nbStates=4, dist=list(step="gamma",angle="vm", dive = 'beta'),
...
m2 <- fitHMM(data=data, nbStates=3, dist=list(step="gamma",angle="vm",dive='pois'),
             Par0=list(step=Par0$Par$step, angle=Par0$Par$angle, dive=divePar0),
...
m3 <- fitHMM(data=data, nbStates=3, dist=list(step="gamma",angle="vm",dive='pois'),
             Par0=list(step=Par0$Par$step, angle=Par0$Par$angle, dive=divePar0),
             beta0=Par0$beta, 
             # fixPar=fixPar,
             # DM=list(angle=list(mean=angleFormula, concentration=~1)),
...
m4 <- fitHMM(data=data, nbStates=3, dist=list(step="gamma",angle="vm",dive='pois'),
             Par0=list(step=Par0$Par$step, angle=Par0$Par$angle, dive=divePar0),
             beta0=Par0$beta, 
             fixPar=fixPar,
             DM=list(angle=list(mean=angleFormula, concentration=~1)),
